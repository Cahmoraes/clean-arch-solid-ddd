...
Atualiza o `UserDetailModal` para incluir a seção "Permissões" com os botões "Tornar Administrador" e "Remover Administrador", seus respectivos AlertDialogs de confirmação e tratamento de erros inline. Reestrutura o footer em duas seções com labels.
...
- react: componentes pequenos com responsabilidade única
- shadcn: usar `AlertDialog` com `AlertDialogAction asChild` para confirmações

## Passos

- [ ] **Step 1: Substituir o conteúdo de `user-detail-modal.tsx`**

Reescreva `apps/frontend/src/features/admin/components/user-detail-modal.tsx` com o conteúdo abaixo. As principais mudanças são:
- `UserStatusActions` não retorna mais `<DialogFooter>` — apenas um `<div>`
- Novo componente `UserPermissionsActions` com os botões de role
- Novo componente `UserActionsSection` que agrupa os dois em `<DialogFooter>` com labels
- Dois novos `AlertDialog`s: `PromoteConfirmationDialog` e `DemoteConfirmationDialog`
- `resolveErrorMessage` agora inclui erros de role
- `UserDetailModal` orquestra os novos estados e handlers
...
interface UserActionsSectionProps extends UserStatusActionsProps, UserPermissionsActionsProps {}
...
export function UserDetailModal({ user, open, onClose }: UserDetailModalProps) {
  ...
  const errorMessage = resolveErrorMessage({
    activateError: activateUser.isError
      ? (activateUser.error?.userMessage ?? null)
      : null,
    suspendError: suspendUser.isError
      ? (suspendUser.error?.userMessage ?? null)
      : null,
    promoteError: promoteToAdmin.isError
      ? (promoteToAdmin.error?.userMessage ?? null)
      : null,
    demoteError: demoteFromAdmin.isError
      ? (demoteFromAdmin.error?.userMessage ?? null)
      : null,
  })
...
```bash
cd apps/frontend
git add src/features/admin/components/user-detail-modal.tsx
git commit -m "feat(admin): add permissions section to UserDetailModal with promote/demote actions"
```

- [ ] **Step 6: Verificação final — build completo**
...
Esperado: tudo passa com zero erros.

## Critérios de Sucesso

- Seção "Permissões" aparece no modal [RF-014]
- "Tornar Administrador" visível para `status=activated` + `role=MEMBER` [RF-015]
- "Remover Administrador" visível para `role=ADMIN` + não-self + não super admin [RF-016]
- Admins não têm botão "Inativar" (regra existente mantida) [RF-017]
- Ambas as ações exigem confirmação via AlertDialog [RF-002, RF-009]
- Erros exibidos inline no modal
- `tsc:check`, `lint:fix`, `test` e `build` passam
