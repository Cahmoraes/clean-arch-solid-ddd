...
- [ ] 1. Domain events novos: `LoginSucceededEvent`, `UserRoleChangedEvent`, `UserStatusChangedEvent` [FR-005, FR-010, FR-011] → `task-01.md`
...
- [ ] 4. `update-my-profile.usecase.ts` religa `UserProfileUpdatedEvent` [FR-009] → `task-04.md`
...
- [ ] 11. Modelo `UserActivityEvent` (Prisma + migration) + `UserActivityRepository` (interface + implementação Prisma) [FR-001] → `task-11.md`
...
- [ ] 17. `ActivityTab` — agrupamento por data + ícone por categoria [FR-002, FR-003, FR-004, FR-013] → `task-17.md`
