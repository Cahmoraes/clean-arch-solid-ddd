...
| Acessibilidade | Já existem padrões de acessibilidade compartilhados (`acessibilidade-frontend` D2) que não podem regredir | `role="grid"`/`gridcell`, `aria-live`, `aria-current`, anel de foco duplo continuam presentes e testados após a mudança |
...
- `apps/frontend/src/features/calendario-feriados/ui/monthly-calendar.tsx` — ajuste de
  `min-h` das células e revisão do uso de `--color-primary` para garantir que só apareça em
  "hoje"/"feriado".
...
|---|---|---|---|---|
| `getWeekOfMonth` calcular semana errada em transições de mês (ex.: dia 1 cai no meio de uma semana) | 2 | 2 | 4 🟡 | Teste unitário cobrindo início/fim de mês em diferentes dias da semana |
...
- Novo teste unitário para `getWeekOfMonth` (casos de borda: primeiro dia do mês em cada dia
  da semana, mês com 28/29/30/31 dias).
- Atualização dos testes existentes de `monthly-calendar` e `holiday-list` para refletir a
  nova estrutura de agrupamento e o uso restrito da cor primária.
