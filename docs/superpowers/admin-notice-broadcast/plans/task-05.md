...
**Status:** DONE
**Verified:** `pnpm --filter backend exec vitest --run --config ./test/vite.config.business-flow.ts src/notification/infra/controller/broadcast-notice.controller.business-flow-test.ts` → exit 0
