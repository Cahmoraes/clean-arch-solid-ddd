...
;(async () => {
	...
	try {
		...
		await page.getByRole("button", { name: /inativos/i }).click()
		await page.waitForTimeout(1000)

		const rows = await page.locator('[data-testid^="user-row-"]').all()
		if (rows.length < 2) {
			throw new Error(`Need >= 2 inactive users, found ${rows.length}`)
		}
		...
		await page.screenshot({
			path: "docs/superpowers/bulk-user-status-actions/qa/evidence/us-05-administrador-ver-ao-final-da/screenshot.png",
			fullPage: true,
		})
