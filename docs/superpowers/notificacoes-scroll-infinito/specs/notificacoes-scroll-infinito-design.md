...
updated_at: "2026-09-07T11:17:16-03:00"
---
...
- **Validação de contrato:** `offset` aceita apenas inteiros `>= 0`; `limit` aceita apenas inteiros entre `1` e `50` (teto explícito contra um cliente pedindo um lote arbitrariamente grande). Fora desses limites, o schema Zod do controller rejeita a requisição com 400.
...
- **Ciclo de vida:** o `IntersectionObserver` é criado e conectado (`observe`) quando a sentinela monta e precisa ser desconectado (`disconnect`) no cleanup do `useEffect` — o dropdown é montado/desmontado a cada abertura/fechamento, então um observer sem cleanup se acumula a cada toggle.
- **Acessibilidade mínima (escopo desta feature):** apesar de acessibilidade além do já existente estar fora de escopo (ver Características Arquiteturais), os elementos *novos* introduzidos por esta feature — sentinela e spinner de rodapé — não podem ficar sem tratamento nenhum, sob risco de regressão silenciosa para leitor de tela: a sentinela é `aria-hidden="true"` (não é conteúdo, não deve ser focável/lida) e o spinner de rodapé usa `role="status"`/`aria-live="polite"` para anunciar o carregamento. Isso é o mínimo necessário para não introduzir uma regressão nova — não uma auditoria de acessibilidade do componente existente.
...
- **Decisão:** o handler de mensagens SSE (`handleNotificationStreamMessage`, definido em `use-notifications.ts` e passado como `onMessage` para `useNotificationStream`) passa a usar `queryClient.setQueryData` para inserir a notificação recebida no início de `data.pages[0].notifications`, sem tocar nas demais páginas. Antes de inserir, o updater verifica se já existe um item com o mesmo `id` em qualquer página já carregada (`data.pages.some(...)`); se existir, o evento é ignorado — essa checagem de duplicidade faz parte do mecanismo, não é apenas uma mitigação de risco documentada à parte. `use-notification-stream.ts` continua responsável apenas por conectar ao SSE e repassar mensagens já parseadas — não é modificado. O contador de não lidas continua sendo invalidado via `invalidateQueries` normalmente — apenas a lista paginada passa a usar `setQueryData`; as mutações `markAsRead`/`markAllAsRead` continuam usando a mesma função de invalidação (`invalidateNotifications`) em `onError`/`onSettled`, que não muda.
- **Contabilização do offset:** a notificação inserida via SSE não deve contar para o cálculo de `offset` do próximo `fetchNextPage` nem inflar o `total` de página — ambos continuam refletindo apenas o que já foi de fato buscado da API. Concretamente, cada página guarda também quantos itens vieram da própria resposta da API (`fetchedCount`), e é essa contagem — não o tamanho do array `notifications` após a inserção do SSE — que alimenta `getNextPageParam`. Sem essa distinção, um item prependado pelo SSE desalinha o próximo `offset` calculado e faz o cliente pular (ou nunca buscar) um item real do backend.
...
- **Observabilidade:** silencioso para o usuário não é o mesmo que invisível para o time — quando as tentativas de retry se esgotam, o erro é registrado via `logger.error` (`apps/frontend/src/lib/observability.ts`, já usado no projeto), sem nenhuma UI adicional. Isso permite detectar uma regressão real (ex.: `offset`/`limit` quebrando em produção após o deploy de D0) sem depender de reclamação orgânica de usuário.
...
| `InMemoryNotificationRepository` (`notification/infra/repository/in-memory/in-memory-notification.repository.ts`) | Mesmo branching de `skip`/`take` do repositório Prisma, usado pelos testes de unidade do use case (`AGENTS.md` do backend exige repositório in-memory, não Prisma, para testes de unidade) | — | `GetNotificationsUseCase` (nos testes) |
| `useNotifications` (`lib/notifications/use-notifications.ts`) | Buscar e paginar notificações via `offset`/`limit`, expor lista achatada + `fetchNextPage`/`hasNextPage`/`isFetchingNextPage`. As funções de atualização otimista do cache (`markAsRead`/`markAllAsRead`) passam a operar sobre `InfiniteData<NotificationsPage>` (`{pages, pageParams}`, onde cada página soma `fetchedCount` a `NotificationsResponse` — ver D2) em vez do objeto único de hoje, já que o formato do cache muda com a migração para `useInfiniteQuery` | Cliente API tipado (`@repo/api-types`, regenerado após a mudança de backend) | `NotificationDropdown` |
...
- **Frontend, unitário (Vitest, `pnpm --filter frontend test -- --run`):** `use-notifications.test.tsx` cobrindo carga inicial (`offset=0&limit=10`), busca de próxima página (`offset=10&limit=5`), fim da paginação (`hasNextPage=false` quando acumulado ≥ `total`), inserção via SSE sem afetar páginas já carregadas, deduplicação por id de uma notificação SSE já presente no cache, e — cobrindo o risco de corrida da tabela de Riscos — uma notificação SSE chegando enquanto um `fetchNextPage` ainda está em andamento (sem duplicar nem corromper a página em anexação). Mocks via `vi.mock("@/lib/api")`, seguindo o padrão já usado nesses testes (sem introduzir MSW). `use-notification-stream.ts` não é modificado nesta feature, então `use-notification-stream.test.ts` não precisa mudar.
- **Componente:** teste de `notification-dropdown.tsx` cobrindo o disparo do sentinela (mock de `IntersectionObserver`, ausente hoje no setup de testes — precisa ser adicionado) chamando `fetchNextPage`, exibição/ocultação do spinner de rodapé conforme `isFetchingNextPage`/`hasNextPage`, e o `disconnect()` do observer sendo chamado no unmount do componente.
