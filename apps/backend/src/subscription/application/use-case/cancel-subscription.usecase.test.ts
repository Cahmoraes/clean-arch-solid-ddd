...
import type { CancelSubscriptionUseCase } from "./cancel-subscription.usecase"
...
describe("CancelSubscriptionUseCase", () => {
	...
	beforeEach(async () => {
		...
		sut = container.get(SUBSCRIPTION_TYPES.USE_CASES.CancelSubscription)
	})
	...
	it("deve cancelar subscription e suspender usuário quando encontrados", async () => {
	...
	it("deve ser idempotente: cancelar subscription já cancelada não causa erro", async () => {
		const userOrError = await User.create({
			id: "user-id-2",
			name: "any_name",
