...
import type { ReactivatePlanUseCase } from "../../../application/use-case/reactivate-plan.usecase.js"
...
export class ReactivatePlanController extends BaseController {
	...
	private async callback(req: FastifyRequest) {
		...
		const result = await this.reactivatePlan.execute(
...
function makeReactivatePlanSwaggerSchema(): Schema {
	return OpenApiSchemaBuilder.build({
		...
		description:
			"Marks a previously inactivated plan as active. Requires ADMIN role",
		security: true,
