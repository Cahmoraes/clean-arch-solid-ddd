...
export function useDemoteFromAdmin(): UseMutationResult<
	void,
	ApiError,
	string
> {
	...
	return useMutation<void, ApiError, string, Context>({
		mutationFn: async (userId: string) => {
			const { error } = await (api.PATCH as any)("/users/demote-admin", {
				body: { userId },
			})
				...
				(old) => {
					...
					return {
						...
						users: old.users.map((user) =>
							user.id === userId ? { ...user, role: "MEMBER" as const } : user,
						),
					}
				},
			)
