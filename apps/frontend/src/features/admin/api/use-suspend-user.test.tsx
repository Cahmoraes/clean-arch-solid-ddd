...
describe("useSuspendUser", () => {
	test("aplica optimistic update para 'suspended' antes de a requisição completar", async () => {
		const queryClient = makeQueryClient()
		const initialData: UseUsersResult = {
			users: [ACTIVATED_USER],
		...
		server.use(
			http.patch(`${apiBaseUrl}/users/suspend`, async () => {
	...
	test("restaura o status anterior no cache em caso de erro da API", async () => {
		...
		expect(restoredData?.users[0]?.status).toBe("activated")
	})
