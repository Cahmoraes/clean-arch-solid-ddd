...
const _NOMINATIM_SEARCH_URL = "https://nominatim.openstreetmap.org/search"
const _NOMINATIM_REVERSE_URL = "https://nominatim.openstreetmap.org/reverse"
...
describe("useGymLocationPicker", () => {
	...
	describe("handleSearch", () => {
		...
		it("define searchError quando endereço não é encontrado", async () => {
			...
			const { result } = renderHook(() => useGymLocationPicker())
			act(() => {
				result.current.handleAddressChange("endereço inexistente xpto")
			})
		...
		it("define searchError em caso de falha de rede", async () => {
			...
			const { result } = renderHook(() => useGymLocationPicker())
			act(() => {
				result.current.handleAddressChange("Av. Paulista")
			})
