...
describe.only('CircuitBreaker', () => {
  ...
  test('Deve causar 50% de falha', async () => {
    ...
    expect(circuitBreaker['exceedFailureThreshold']).toBe(false)
  })
  ...
  test('Deve causar entrar em estado half-open', async () => {
    ...
    expect(circuitBreaker['failureThresholdPercentage']).toBe(75)
    expect(circuitBreaker['exceedFailureThreshold']).toBe(true)
